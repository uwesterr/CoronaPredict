...
calcWirksamkeitReduktion <- function(calcDf, reduzierung_datum, ta) {
  if (calcDf$Tag < reduzierung_datum){
    ...
    WirksamkeitReduktion <-min(1,as.numeric(calcDf$Tag - reduzierung_datum)/ta)
...
```{r updateCalcDf, eval=FALSE, include=FALSE}
...
updatecalcDf <- tibble(
       ...
       TaeglichReproduktionsRateRt       = calcTaeglichReproduktionsRateRt(Rt, calcDf, Y_inf_limit),
       AktuellInfizierteBerechnet        = n0_erfasst,
...
updatecalcDf$WirksamkeitReduktion<- calcWirksamkeitReduktion(tailCalcDf, reduzierung_datum, ta)  
updatecalcDf$ReduzierteRt<- calcReduzierteRt(tailCalcDf)
